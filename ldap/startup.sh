...
if [ ! -e "$FIRST_START_DONE" ]; then
  ...
  function ldap_add_or_modify (){
    ...
    sed -i "s|{{ OPENFIRE_ADMIN_GROUP }}|${OPENFIRE_ADMIN_GROUP}|g" $LDIF_FILE
    sed -i "s|{{ OPENFIRE_ADMIN_FULL_NAME }}|${OPENFIRE_ADMIN_FULL_NAME}|g" $LDIF_FILE
    sed -i "s|{{ OPENFIRE_ADMIN }}|${OPENFIRE_ADMIN}|g" $LDIF_FILE
    ENCRYPTED_PASSWD=`perl /hashit.pl ${OPENFIRE_ADMIN_PASSWORD}` 
    sed -i "s|{{ ENCRYPTED_ADMIN_PASSWORD }}|${ENCRYPTED_PASSWD}|g" $LDIF_FILE
    cp $LDIF_FILE /
